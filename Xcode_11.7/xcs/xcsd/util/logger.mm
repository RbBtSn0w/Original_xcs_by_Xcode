#include <asl.h>
#include <nan.h>
#include <os/log.h>
#include <stdio.h>

#import <Foundation/Foundation.h>

using namespace v8;

static BOOL useTraceLogging;
static asl_object_t logClient;
static os_log_t traceLogClient;

NAN_METHOD(LogMessage) {
    Local<Integer> level = info[0].As<Integer>();
    String::Utf8Value message(info[1]);
    Local<Object> props = info[2].As<Object>();

    if (useTraceLogging) {
        String::Utf8Value requestID(props->Get(Nan::New("RequestID").ToLocalChecked()));

        if (level->Value() == ASL_LEVEL_CRIT || level->Value() == ASL_LEVEL_ERR) {
            os_log_error(traceLogClient, "[%{public}s] %{public}s", *requestID, *message);
        } else if (level->Value() == ASL_LEVEL_INFO) {
            os_log_info(traceLogClient, "[%{public}s] %{public}s", *requestID, *message);
        } else if (level->Value() == ASL_LEVEL_DEBUG) {
            os_log_debug(traceLogClient, "[%{public}s] %{public}s", *requestID, *message);
        } else {
            os_log(traceLogClient, "[%{public}s] %{public}s", *requestID, *message);
        }
    } else {
        Local<Array> propertyNames = props->GetOwnPropertyNames();
        aslmsg msg = asl_new(ASL_TYPE_MSG);
        for (uint32_t i = 0; i < propertyNames->Length(); i++) {
            Local<Value> keyValue = propertyNames->Get(i);
            String::Utf8Value key(keyValue);
            String::Utf8Value value(props->Get(keyValue));

            asl_set(msg, *key, *value);
        }

        asl_log(logClient, msg, level->Value(), "%s", *message);
    }

    info.GetReturnValue().SetUndefined();
}

NAN_METHOD(SetLogFacility) {
    String::Utf8Value newFacilityName(info[0]);

    if (useTraceLogging) {
        traceLogClient = os_log_create(*newFacilityName, "messages");
    } else {
        if (logClient) {
            asl_close(logClient);
        }

        logClient = asl_open(NULL, *newFacilityName, 0);
        asl_set_filter(logClient, ASL_FILTER_MASK_UPTO(ASL_LEVEL_DEBUG));
    }

    info.GetReturnValue().SetUndefined();
}

NAN_MODULE_INIT(LoggerInit) {
    NSOperatingSystemVersion mac1012 = { 10, 12, 0 };
    useTraceLogging = [[NSProcessInfo processInfo] isOperatingSystemAtLeastVersion:mac1012];

    Nan::SetMethod(target, "setFacility", SetLogFacility);
    Nan::SetMethod(target, "logMessage", LogMessage);
}

NODE_MODULE(logger, LoggerInit)
