...
XCSIssueClass.prototype.createIssue = function createIssue(req, integration, issue, cb) {
    ...
    async.waterfall([
        ...
        function ISCreateIssueHashForIssue(processedIssue, callback) {
            hashForIssue(req, issue, function ISCreateIssueHashForIssueCallback(err, hash) {
                if (err) {
                    return xcsutil.safeCallback(callback, err);
                } else {
                    return xcsutil.safeCallback(callback, null, processedIssue, hash);
                }
            });
        },
        function ISCreateIssueFindIssuesByHash(processedIssue, hash, callback) {
        ...
        resolvedIssuesForIntegration(req, integration, issueClass, (err, issues) => {
            ...
            return cb(null, issues.map(issue => {
                ...
                return {
                    ...
                    issueAuthors: issue.issueAuthors,
