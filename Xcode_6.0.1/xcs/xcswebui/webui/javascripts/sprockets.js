...
XCS.WebUI.WebUIController = Class.create(XCS.Mvc.ViewController, {
	...
	load: function(inFilter, inBotTinyId, inRoutePattern) {
		...
		this.getCommitsAndIssuesForFileterdIntegrations();
	...
	getUserAuthStatus: function() {
		...
		dispatch_async(this.mQueue, function(manager) {
			...
			var isLoggedIn = null;
			var isBotCreator = null;
			
			var loggedInCallback = function(inIsLoggedIn) {
				response_count++;
				isLoggedIn = inIsLoggedIn;
	...
	getCommitsAndIssuesForFileterdIntegrations: function() {
		dispatch_async(this.mQueue, function(manager) {
			var maxCount = Object.keys(this.mBotsDict).length;
			var count = 0;
...
XCS.WebUI.Views.BotListItemView = Class.create(XCS.Mvc.View, {
	...
	updateSpinner: function(inIntegration) {
		...
		if (integration !== undefined && integration !== null) {
			if (integration.isStepGreaterOrEqualThan(XCS.Helpers.INTEGRATION_CURRENT_STEP_PREPARING) && !integration.isStepGreaterOrEqualThan(XCS.Helpers.INTEGRATION_CURRENT_STEP_COMPLETED)) {
				this.showSpinner();
			}
			else {
				this.hideSpinner();
