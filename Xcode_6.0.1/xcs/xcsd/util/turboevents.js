            ...
            if (msg.command)
            {
                ...
                else if (msg.command === 'TurboEventBroadcastWithCallback')
                {
                    self.internalInquire(msg.filter, msg.event, msg.data, function(response){
                        process.send({command: 'TurboEventResponse', identifier: msg.identifier, worker: msg.worker, data: response});
                    });
                }
                else if (msg.command === 'TurboEventResponse')
                {
...
TurboEventManager.prototype.registerTurboSocket = function(socket){
    var s = new TurboEventSocket(socket, 'turbo', this);
    this.register(s);
};

TurboEventManager.prototype.registerSocketIOSocket = function(socket){
    var s = new TurboEventSocket(socket, 'socket.io', this);
    this.register(s);
};

TurboEventManager.prototype.register = function(socket){
...
TurboEventManager.prototype.broadcast = function(filter, event, data){
    if (cluster.isDisabled) return this.internalBroadcast(filter, event, data);
    if (cluster.isMaster)   return;
    
    process.send({command: "TurboEventBroadcast", filter: filter, event: event, data: data});
};

TurboEventManager.prototype.inquire = function(filter, event, data, cb){
    if (cluster.isDisabled) return this.internalInquire(filter, event, data, cb);
    if (cluster.isMaster)   return;
    
    var identifier = uuid.v4();
    this.inquireCallbacks[identifier] = cb;
    
    process.send({command: "TurboEventBroadcastWithCallback", filter: filter, event: event, data: data, identifier: identifier});
};

TurboEventManager.prototype.internalBroadcast = function(filter, event, data){
...
TurboEventManager.prototype.internalInquire = function(filter, event, data, cb){
    var sockets = this.sockets;
    var filterFn = (filter) ? require('../socket.js')[filter] : null;
    if (filterFn)
        sockets = sockets.filter(filterFn);
...
TurboEventManager.prototype.connectionStats = function(){
    var connections = {"turbosocket": 0, "socketio": 0};
...
TurboEventSocket.prototype.on = function(event, cb){
...
TurboEventSocket.prototype.emit = function(event, data, cb){
    if (this.mode == 'socket.io')
    ...
    else if (this.mode == 'turbo')
    {
        ...
        function prepareMessage(err, theUUID) {
            var theEvent = {name: event, data: data};
            if (cb)
            {
                if (err)
                {
                    konsole.error('Error obtaining UUID: ' + err);
                    return;
                }
            ...
            try {
                ...
                konsole.warn('[TurboEvents] Could not serialize a message, skipping');
                return;
            }
...
TurboEventSocket.prototype.internalEmit = function(eventName){
...
TurboEventSocket.prototype.handleEvent = function(eventName, data, callbackUUID){
    var observers = this.observers[eventName];
    var self = this;
    ...
    if (observers)
    {
        for (var i = 0; i < observers.length; i++)
        {
            ...
            {
                observers[i](data, function(responseData){
...
TurboEventSocket.prototype.handleCallback = function(callbackUUID, data){
...
TurboEventSocket.prototype.handleDatagram = function(datagram, tag){
    var data = null;
    try {
        data = JSON.parse(datagram.toString('utf8'));
    } catch (e) {
        konsole.warn('[TurboEvents] Could not deserialize a message, skipping');
        return;
    }
...
TurboEventSocket.prototype.sendResponse = function(callbackUUID, theData){
    var theResponse = {callback: callbackUUID};
    if (theData)
    ...
    try {
        ...
        konsole.warn('[TurboEvents] Could not serialize a response, skipping');
        return;
    }
...
TurboEventSocket.prototype.handleAuthentication = function(username, password, callback){
    this.internalEmit('authenticate', username, password, function(err){
        callback(err);
    });
};

TurboEventSocket.prototype.handlePrivilegeChange = function(username){
