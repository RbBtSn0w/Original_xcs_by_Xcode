    ...
    xcsSecurityQueue = async.queue(function (task, cb) {
        ...
        function attemptXCSSecurityInvocation(err) {
            ...
            if (numberOfXCSSecurityInvocationAttempts > 3) {
                var error = {
                    status: 500,
                    message: 'Internal Server Error: XCSSecurity failed after 3 attempts.'
                };

                // Check if we're carrying an error from an previous invocation
                if (err) {
                    error.message += 'Reason: ' + err.message;
                }

                logger.error('Exception running xcssecurity:', error);
            ...
            try {
                ...
                xcssecurity.on('close', function (status) {
                    ...
                    */
                    ...
                    if (0 === httpStatus) {
                        ...
                        error = {
                            status: httpStatus,
                            message: ((errors.length > 0) ? errors[errors.length - 1] : 'Unexpected error in security operation')
                        };
                ...
                var someError = {
                    status: 500,
                    message: 'Exception invoking xcssecurity: ' + e.toString()
                };
                logger.error(someError.message);
                return xcsutil.safeCallback(cb, someError);
            }
        }
...
Identity.prototype.signMessage = function signMessage(inputData, callback) {
    ...
    launchTool(['message-sign', '-i', '-'], env, this.keychain, inputData, function (err, output) {
        xcsutil.profilerSummary(null);
        if (err) {
            err.message = 'Error signing message: ' + err.message;
            logger.error(err);
            return xcsutil.safeCallback(callback, err);
        } else {
            return xcsutil.safeCallback(callback, null, output);
