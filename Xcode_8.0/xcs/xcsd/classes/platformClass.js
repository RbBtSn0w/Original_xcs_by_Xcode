...
XCSPlatformClass.prototype.list = function (req, res) {
    this.listPlatforms(req, (err, docs) => {
        if (err) {
...
XCSPlatformClass.prototype.listPlatforms = function (req, cb) {
    ...
    redisClass.getDynamicQuery(req, doctype, (err, docs) => {
        if (err) {
            cb(err);
        } else if (docs) {
            ...
            dbCoreClass.listAllDocuments(req, doctype, (err, docs) => {
                if (err && err.status !== 404) {
                    cb(err);
                } else {
                    log.info('Found', docs.length, 'platforms in CouchDB.');
                    redisClass.setDynamicQuery(req, doctype, JSON.stringify(docs), () => {
                        cb(null, docs);
                    });
                }
            });
    ...
    async.waterfall([
        ...
        if (!err) {
            ...
            redisClass.delDynamicQuery(req, k.XCSDesignDocumentPlatform, function () {
                te.broadcast(k.XCSIsListenerForDeviceUpdates, k.XCSEmitNotificationPlatformUpdated, savedPlatform);
