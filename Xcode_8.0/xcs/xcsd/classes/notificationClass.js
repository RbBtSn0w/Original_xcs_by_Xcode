...
XCSNotificationClass.prototype.sendNotifications = function sendNotifications(req, res) {
    ...
    var integrationId = (req && req.params && req.params.id) || null;
    var trigger = (req && req.body && req.body.trigger) || null;
...
XCSNotificationClass.prototype.sendBotReport = function sendBotReportEndpoint(req, res) {
    ...
    var schedule = (req && req.body && req.body.schedule) || null;
    var botID = (req && req.params && req.params.id) || null;
    var recipients = (req && req.body && req.body.recipients) || null;
    var date = (req && req.body && req.body.date) || null;
    var rawContent = (req && req.body && req.body.rawContent) || null;
...
XCSNotificationClass.prototype.sendIntegrationReport_internal = function sendIntegrationReport_internal(req, integrationID, trigger, cb, recipients, opts) {
    async.parallel({
        ...
        if (err) {
            ...
            results.schedule = "integration";
            var integration = results.integration;
...
XCSNotificationClass.prototype.integrationEmail = function (info, cb) {
    var subject = templates.subject(info);
    var text = templates.text(info);
    var html = templates.html(info);
    var from = "";
    var ccAddresses = [];
    var replyToAddress = "";
...
XCSNotificationClass.prototype.newIssuesEmails = function (info, cb, opts) {
    var emails = [];

    if (Object.keys(info.issuesByAuthor).length > 0) {
        for (var issueAuthorKey in info.issuesByAuthor) {
            if (info.issuesByAuthor[issueAuthorKey] !== undefined) {

                var issueAutor = info.issuesByAuthor[issueAuthorKey];
                if (issueAutor !== undefined && issueAutor !== null) {
                    var recipient = (issueAutor && issueAutor.contributor && issueAutor.contributor.XCSContributorEmails && issueAutor.contributor.XCSContributorEmails.length && issueAutor.contributor.XCSContributorEmails[0]);
                    var data = {
                    ...
                    var subject = this.getNewIssueEmailSubject(issueAutor, info.integration);
                    var text = this.getNewIssueEmailText(data);
                    var html = newIssueTemplate.html(data);
...
XCSNotificationClass.prototype.reportEmail = function reportEmail(info, cb, opts) {
    var emails = [];
    ...
    if (info && info.recipients) {
        ...
        var text = this.getReportEmailText(data);
        var html = reportTemplate.html(data);
        ...
        var from = getEmailAddressWithName(data.bot.name, fromAddress);
...
XCSNotificationClass.prototype.prepareResultForNewIssueNotification = function prepareResultForNewIssueNotification(results) {
    if (results !== undefined && results !== null && results.issues && results.issues.errors && results.issues.testFailures && results.issues.warnings && results.issues.analyzerWarnings) {
        ...
        var integration = results.integration;
        var revisionBlueprint = integration.revisionBlueprint;
        var types = ['error', 'testFailure', 'warning', 'analyzerWarning'];
        var allFreshIssues = _.flatten(types.map(function (type) {
        ...
        for (var i = 0; i < allFreshIssues.length; i++) {
            var issue = allFreshIssues[i];
            var issueAuthors = issue.issueAuthors;

            // For all the issueAuthors
            for (var j = 0; j < issueAuthors.length; j++) {
                var issueAuthor = issueAuthors[j];

                // We grab the full commit data from the issue commits array
                var hydratedCommit = (commitsByHash[issueAuthor.XCSCommitHash] || null);

                if (hydratedCommit !== undefined && hydratedCommit !== null) {
                    // Find author information from the commit data
                    var authorHash = getContributorHash(hydratedCommit);
                    ...
                    if (issuesByAuthor[authorHash].issues[issue._id] === undefined) {
                        issuesByAuthor[authorHash].issues[issue._id] = {
                            issue: issue,
                            authorStrategy: []
                        };
                    }
                    ...
                    if (revisionBlueprint !== undefined && revisionBlueprint !== null) {
                        ...
                        for (var w = 0; w < revisionBlueprint.DVTSourceControlWorkspaceBlueprintRemoteRepositoriesKey.length; w++) {
                            var remoteRepository = revisionBlueprint.DVTSourceControlWorkspaceBlueprintRemoteRepositoriesKey[w];
                        ...
                        for (var repositoryId in revisionBlueprint.DVTSourceControlWorkspaceBlueprintLocationsKey) {
                    ...
                    issuesByAuthor[authorHash].issues[issue._id].authorStrategy.push(issueAuthor.XCSIssueSuspectstrategy);
                }
            }
        }

        // Filter out excluded repositories and excluded domain names
        var emailConfiguration = results.trigger && results.trigger.emailConfiguration;
        // First, we exclude recipients with non allowed domain names
        if (emailConfiguration.allowedDomainNames.length > 0) {
            for (var issueAuthorHash1 in issuesByAuthor) {
                if (issueAuthorHash1 && issuesByAuthor[issueAuthorHash1] !== undefined) {
                    var issueAuthor1 = issuesByAuthor[issueAuthorHash1];
                    var contributor1 = issueAuthor1.contributor;
                    var contributorEmail1 = contributor1.XCSContributorEmails && contributor1.XCSContributorEmails.length && contributor1.XCSContributorEmails[0];
        ...
        for (var issueAuthorHash2 in issuesByAuthor) {
            if (issueAuthorHash2 && issuesByAuthor[issueAuthorHash2] !== undefined) {
                var issueAuthor2 = issuesByAuthor[issueAuthorHash2];
                var issues2 = issueAuthor2.issues;

                for (var issuesId2 in issues2) {
                    if (issuesId2 && issues2[issuesId2] !== undefined) {
                        var shouldDeleteIssue = false;
                        var issue2 = issues2[issuesId2].issue;
                        var authorStrategies2 = issues2[issuesId2].authorStrategy;

                        // Exclude issue if found in exluded repository
                        for (var i2 = 0; i2 < authorStrategies2.length; i2++) {
                            var authorStrategy2 = authorStrategies2[i2];
                            var commit2 = authorStrategy2.commit;
        ...
        for (var mappedIssueAuthorHash in issuesByAuthor) {
            if (issuesByAuthor[mappedIssueAuthorHash] !== undefined) {
                var mappedIssueAuthor = issuesByAuthor[mappedIssueAuthorHash];
                var highConfidenceIssues = [];
                var lowConfidenceIssues = [];
                var mappedIssues = mappedIssueAuthor.issues;
                var commitsByRepositoryBranch = {};
                var commitsByRepositoryArray = [];
                var commitsCountDict = {};

                for (var mappedIssueId in mappedIssues) {
                    if (mappedIssues[mappedIssueId] !== undefined) {
                        var mappedIssueContainer = mappedIssues[mappedIssueId];
                        var highStrategyFound = false;
                        var lowStrategyFound = false;

                        mappedIssueContainer.authorStrategy.sort(this.sortAuthorStrategies);

                        for (var p = 0; p < mappedIssueContainer.authorStrategy.length; p++) {
                            var authorStrategy = mappedIssueContainer.authorStrategy[p];
                ...
                mappedIssueAuthor.issueIdsWithHighStrategy = highConfidenceIssues;
                mappedIssueAuthor.issueIdsWithLowStrategy = lowConfidenceIssues;
                mappedIssueAuthor.issuesCount = highConfidenceIssues.length + lowConfidenceIssues.length;
                mappedIssueAuthor.commitsCount = Object.keys(commitsCountDict).length;
                ...
                mappedIssueAuthor.numberOfIssuesByCommit = commitsByRepositoryArray;
            }
        }
...
XCSNotificationClass.prototype.prepareResultForReportNotification = function prepareResultForReportNotification(results) {
    ...
    var testsDelta = 0;
    var testsFailureDelta = 0;
    var perfTestsRegressionDelta = 0;
    var codeCoveragePercentageDelta = 0;
    var maximumNumberOfTests = {
        count: 0,
        integrationNumber: 0
    };
    var commitsCount = 0;
    ...
    var lowestIntegrationNumber = 0;
    var commitsByRepositoryBranch = {};
    var commitsByRepositoryArray = [];
    var commitsByHash = {};
    var integrationsCount = 0;
    var lastIntegration = null;
    var integrationCommitsShas = [];

    if (results !== undefined && results !== null && results.integrations) {
        var integrations = results.integrations;
        //var recipients = [];
        var sendEmailToCommitters = false;
        ...
        if (results.recipients === undefined && results.trigger && results.trigger.emailConfiguration) {
            var emailConfiguration = results.trigger.emailConfiguration;
        ...
        for (var i = 0; i < integrations.length; i++) {
            ...
            var issues = (integration && integration.issues);
            var commits = (integration && integration.commits);
            ...
            if (commits) {
                for (var m = 0; m < commits.length; m++) {
                    var commit = commits[m];
                    var innerCommits = commit.commits;

                    for (var repoID in innerCommits) {
                        if (innerCommits[repoID] !== undefined) {
                            var commitsArray = innerCommits[repoID];

                            for (var p = 0; p < commitsArray.length; p++) {
                                var xcsCommit = commitsArray[p];
                                commitsCount += 1;
                                ...
                                if (sendEmailToCommitters) {
                                    if (isBlueprintIdAllowed(xcsCommit.XCSBlueprintRepositoryID, results.trigger.emailConfiguration.scmOptions)) {
                                        var email = getContributorEmail(xcsCommit);
        ...
        for (var issueId in issuesDict) {
            if (issueId && issuesDict[issueId] !== undefined) {
                var issuesArray = issuesDict[issueId];

                for (var c = 0; c < issuesArray.length; c++) {
                    var issueDict = issuesArray[c];
                    var issue1 = issueDict.issue;
                    var integration1 = issueDict.integration;
                    ...
                    if (issue1.issueAuthors && issue1.issueAuthors.length > 0) {
                        for (var h = 0; h < issue1.issueAuthors.length; h++) {
                            var issueAuthor = issue1.issueAuthors[h];
                            this.processIssueAuthorRepository(issueAuthor, integration1);

                            if (commitsByRepositoryBranch[issueAuthor.repository + issueAuthor.branch] && commitsByRepositoryBranch[issueAuthor.repository + issueAuthor.branch].commits[issueAuthor.XCSCommitHash] && issue1.cause === undefined) {
        ...
        if (results.trigger && results.trigger.emailConfiguration) {
            var newListOfRecipients = [];
            for (var r = 0; r < results.recipients.length; r++) {
                var recipient = results.recipients[r];
            ...
            if (results.trigger.emailConfiguration.ccAddresses) {
                var newListOfCCRecipients = [];
                for (var t = 0; t < results.trigger.emailConfiguration.ccAddresses.length; t++) {
                    var ccRecipient = results.trigger.emailConfiguration.ccAddresses[t];
...
XCSNotificationClass.prototype.getNewIssueEmailSubject = function getNewIssueEmailSubject(issueAuthor, integration) {
    if (issueAuthor !== undefined && issueAuthor !== null) {
        ...
        if (issueAuthor.issueIdsWithHighStrategy.length > 0) {
            emailSubject += "You introduced " + pluralize(issueAuthor.issueIdsWithHighStrategy.length, "issue");
        }
        if (issueAuthor.issueIdsWithHighStrategy.length > 0 && issueAuthor.issueIdsWithLowStrategy.length > 0) {
            emailSubject += " and ";
        }
        if (issueAuthor.issueIdsWithLowStrategy.length > 0) {
            if (emailSubject === "") {
                emailSubject += "You ";
            }

            emailSubject += "might be able to help solve " + pluralize(issueAuthor.issueIdsWithLowStrategy.length, "issue");
        }
...
XCSNotificationClass.prototype.getReportEmailSubject = function getReportEmailSubject(info) {
    if (info) {
        ...
        if (info.schedule === "daily" || info.schedule === "weekly") {
            ...
            var integration = info.integration;
            bot = integration.bot;
            if (integration) {
...
XCSNotificationClass.prototype.getReportEmailText = function getReportEmailText(info) {
    if (info) {
        ...
        if (info.integrations && info.integrations.length) {
            var integration = info.integrations[0];
...
XCSNotificationClass.prototype.sortByContributor = function sortByContributor(a, b) {
    var contributor_a = a.contributor;
    var contributor_b = b.contributor;
...
XCSNotificationClass.prototype.processCommitRepository = function processCommitRepository(commit, integration) {
    if (commit !== undefined && commit !== null && integration !== undefined && integration !== null) {
        var blueprintLocationsKey = (integration.revisionBlueprint && integration.revisionBlueprint.DVTSourceControlWorkspaceBlueprintLocationsKey) || {};
        var remoteRepositoriesKey = (integration.revisionBlueprint && integration.revisionBlueprint.DVTSourceControlWorkspaceBlueprintRemoteRepositoriesKey) || {};

        var repository = this.getRepositoryURLKeyFromIdentifierKey(commit.XCSBlueprintRepositoryID, remoteRepositoriesKey);
        var branch = this.getBranchIdentifierKeyFromLocationsKey(commit.XCSBlueprintRepositoryID, blueprintLocationsKey);
...
XCSNotificationClass.prototype.processIssueAuthorRepository = function processCommitRepository(issueAuthor, integration) {
    if (issueAuthor !== undefined && issueAuthor !== null && integration !== undefined && integration !== null) {
        ...
        var repository = this.getRepositoryURLKeyFromIdentifierKey(issueAuthor.XCSBlueprintRepositoryID, remoteRepositoriesKey);
        var branch = this.getBranchIdentifierKeyFromLocationsKey(issueAuthor.XCSBlueprintRepositoryID, blueprintLocationsKey);
...
XCSNotificationClass.prototype.createCommitsArrayWithIssuesReferences = function createCommitsArrayWithIssuesReferences(commitsByRepositoryBranch) {
    var commitsByRepositoryArray = [];
    for (var repoUrlBranchName in commitsByRepositoryBranch) {
        if (repoUrlBranchName && commitsByRepositoryBranch[repoUrlBranchName] !== undefined) {
            var commitByRepositoryBranch = commitsByRepositoryBranch[repoUrlBranchName];
            var commitsArray = [];

            for (var commitHash in commitByRepositoryBranch.commits) {
                if (commitHash && commitByRepositoryBranch.commits[commitHash] !== undefined) {
                    var issueTypes = commitByRepositoryBranch.commits[commitHash].issueTypes;
...
XCSNotificationClass.prototype.updateIssueDictToTheMostRecent = function updateIssueDictToTheMostRecent(issuesDict, currentIssue, integration) {
    if (issuesDict && currentIssue) {
        ...
        if (issuesDict[issueID] === undefined) {
            ...
            var savedIssuesArray = issuesDict[issueID];
            if (savedIssuesArray && savedIssuesArray.length > 0) {
                var savedIssue = savedIssuesArray[0];
        ...
        for (var issueId in issuesDict) {
            if (issuesDict[issueId]) {
                ...
                for (var i = 0; i < issuesArray.length; i++) {
                    var issueDict = issuesArray[i];
                    var issue = issueDict.issue;
                ...
                if (undefined === bot[k.XCSUnitTestHeader]) {
                    if (bot && bot.configuration && bot.configuration.triggers) {
                        ...
                        bot.configuration.triggers.forEach(trigger => {
                            ...
                            var task = null;
                            var minuteString = ((emailConfiguration.minutesAfterHour < 10) ? '0' : '') + emailConfiguration.minutesAfterHour;
                            ...
                            // weekly
                                ...
                                var day = (emailConfiguration.weeklyScheduleDay % 7); // wrap around Sunday
...
XCSNotificationClass.prototype.createCommitsByHashDict = function createCommitsByHashDict(issues, integration, commitsByHash) {
    if (issues && integration && commitsByHash) {
        for (var t = 0; t < issues.length; t++) {
            var freshIssue = issues[t];
            
            if (freshIssue.commits) {
                for (var h = 0; h < freshIssue.commits.length; h++) {
                    // We process to commit to capture the repo URL and the name of the branch
                    var commit = freshIssue.commits[h];
...
XCSNotificationClass.prototype.numberOfCommitters = function numberOfCommitters(commitsByHash, integrationCommitsShas) {
    var committersCount = 0;
    var committersDict = {};

    if (commitsByHash && integrationCommitsShas) {
        for (var i = 0; i < integrationCommitsShas.length; i++) {
            var commit = commitsByHash[integrationCommitsShas[i]];
            if (commit) {
                var contributorEmail = getContributorEmail(commit);
...
XCSNotificationClass.prototype.listOfCommitters = function listOfCommitters(commitsByHash, integrationCommitsShas) {
    ...
    if (commitsByHash && integrationCommitsShas) {
        for (var i = 0; i < integrationCommitsShas.length; i++) {
            ...
            if (commit) {
                var contributorEmail = getContributorEmail(commit);
                var contributor = getContributor(commit);
        ...
        for (var contributorEmail1 in committersDict) {
...
XCSNotificationClass.prototype.listOfIssuesSummary = function listOfIssuesSummary(integration) {
    ...
    if (integration && integration.buildResultSummary) {
        ...
        if (buildResultSummary.errorCount > 0) {
            var errorString = notification_helper.pluralize(buildResultSummary.errorCount, "error");
        ...
        if (buildResultSummary.warningCount > 0) {
            var warningString = notification_helper.pluralize(buildResultSummary.warningCount, "warning");
        ...
        if (buildResultSummary.analyzerWarningCount > 0) {
            var analysisString = notification_helper.pluralize(buildResultSummary.analyzerWarningCount, "analysis issue");
        ...
        if (buildResultSummary.testsCount > 0 && buildResultSummary.testFailureCount === 0) {
            var passingTestString = notification_helper.pluralize(buildResultSummary.testsCount, "passing test");
            ...
            var testFailureString = notification_helper.pluralize(buildResultSummary.testFailureCount, "test failure");
            ...
            var testString = notification_helper.pluralize(buildResultSummary.testsCount, "test");
            if (buildResultSummary.testsChange !== 0) {
                testString += " (" + (buildResultSummary.testsChange > 0 ? "+" : "") + buildResultSummary.testsChange + ")";
            }
            summaryArray.push(testString);

            var testPassingString = notification_helper.pluralize(buildResultSummary.testsCount - buildResultSummary.testFailureCount, "passing test");
            summaryArray.push(testPassingString);

            var testFailureString2 = notification_helper.pluralize(buildResultSummary.testFailureCount, "test failure");
            if (buildResultSummary.testFailureChange !== 0) {
        ...
        if (buildResultSummary.codeCoveragePercentage > 0) {
            var codeCoverageString = buildResultSummary.codeCoveragePercentage + "% coverage";
...
function getContributorHash(commit) {
    if (commit) {
        ...
        var hash = contributor.XCSContributorName.replace(/ /g, '').toLowerCase();
        for (var i = 0; i < contributor.XCSContributorEmails.length; i++) {
...
function getContributorEmail(commit) {
    if (commit) {
        var email = "";
        var emails = commit.XCSCommitContributor && commit.XCSCommitContributor.XCSContributorEmails;
...
function isEmailAddressAllowed(email, allowedDomainNames) {
    ...
    if (email && allowedDomainNames) {
        for (var i = 0; i < allowedDomainNames.length; i++) {
            var domainName = allowedDomainNames[i];
...
function orderIssuesByType(issues, issuesDict) {
    ...
    if (issues && issuesDict) {
        var issuesTypeDict = {};
        var types = ['buildServiceError', 'error', 'testFailure', 'triggerError', 'warning', 'analyzerWarning', 'buildServiceWarning', 'unknown'];

        for (var i = 0; i < issues.length; i++) {
            var issueDict = issues[i];
