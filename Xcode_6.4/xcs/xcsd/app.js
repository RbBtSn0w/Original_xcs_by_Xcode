...
require('./classes/couchdbClass.js');

// Redis init ========================================================================

require('./classes/redisClass.js');
...
function startXCS() {
    ...
    konsole.log(null, '*******************************************');
    konsole.log(null, '*********** Load and cache ACLs ***********');
    konsole.log(null, '*******************************************\n');

    redis.client().set(k.XCSRedisACLExpansion, '1');
    ...
    auth.loadAndCacheACLDocumentWithUUID(null, function (err) {
        if (err) {
            ...
            konsole.warn(null, '[ACL loaded and cached] warning: ' + message);
            return;
        } else {
            redis.client().del(k.XCSRedisACLExpansion);
            konsole.log(null, '[ACL loaded and cached] success.');
        }
    });

    // finish bootstrapping: spool up the scheduler for periodic integrations
...
async.series([
        ...
        function (next) {
            ...
            if (cluster.isMaster || cluster.isDisabled) {
                ...
                keychain.exportIdentity(commonName, null, XCS_SSL_SERVER_CERT_PATH, XCS_SSL_SERVER_KEY_PATH, function (err) {
                    if (err) {
                        konsole.log(null, '[XCSNode] SSL: didn\'t find a certificate for ' + commonName + '. Generating a new one.');
                        // generate a new one
                        keychain.generateCSR(commonName, emailAddress, function (err, csr) {
                            if (err) {
                                ...
                                ca.fulfillCSR(csr, keychain, k.XCSSSLCertificateValidityPeriod, function (err) {
                                    if (err) {
                                        ...
                                        keychain.exportIdentity(commonName, emailAddress, XCS_SSL_SERVER_CERT_PATH, XCS_SSL_SERVER_KEY_PATH, function (err) {
                                            if (err) {
        ...
        function (next) {
            ...
            function spawnWorkers(serviceEnabled) {
            ...
            function startAllWorkers(serviceEnabled, skipSetKillAllWorkersTimeout) {
            ...
            if (cluster.isMaster && !cluster.isDisabled) {
                ...
                r.del('cluster:*', function () {
                    ...
                    delegation.cleanAll(function () {
                        ...
                        startAllWorkers(settingsDocument.service_enabled, skipSetKillAllWorkersTimeout);
