...
integration_search.findIntegrationsWithNumbersForBotWithUUID = function (req, integrationNumbers, botUUID, cb) {
    ...
    db_core.findDocumentsWithQuery(req, k.XCSDesignDocumentIntegration, view_byNumber, query, function (err, docs) {
        if (err) {
            konsole.error(req, '[Integration Search - findIntegrationsWithNumbersForBotWithUUID] error: ' + err.message);
            xcsutil.logLevelDec(req);
            return cb(err);
        } else {
            if (docs.length > 0) {
...
integration_search.listRunning = function (req, res) {
    ...
    db_core.findDocumentsWithQuery(req, k.XCSDesignDocumentIntegration, k.XCSDesignDocumentViewIntegrationsRunning, query, function (err, docs) {
        xcsutil.profilerSummary(req);
        xcsutil.logLevelDec(req);
        xcsutil.logLevelCheck(req, logLevel);
        if (err && err.status === 404) {
