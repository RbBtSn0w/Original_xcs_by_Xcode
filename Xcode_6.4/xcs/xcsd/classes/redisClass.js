...
XCSRedisClass.prototype.connectToRedis = function connectToRedis() {
    ...
    self.redisClient = self.Redis.createClient(k.XCSRedisPort, k.XCSRedisHost);
    ...
    self.redisClient.on('end', function REDClientEndEvent(err) {
        if (err) {
            ...
            self.redisLog('[Redis - connectToRedis] ***** Error: ' + reason + '. Redis is not available. Will try to connect in ' + self.delayInMs / 1000 + ' seconds.');
        } else {
            self.redisLog('[Redis - connectToRedis] ***** Error: Redis is not available. Will try to connect in ' + self.delayInMs / 1000 + ' seconds.');
        }
        ...
        if (!self.previouslyInitialized) {
            // We have never been able to connect. If we have tried less than 15 times, continue trying for a little longer
            if (self.numberOfAttempts < 15) {
                self.setRedisConnectionTimeout();
            } else {
                // OK. It seems that Redis is now showing up. After 15 attempts we have to give up and exit because
                // we cannot cache crucial xcsd info we'll need later in the initialization.
                konsole.error(null, '[Redis - connectToRedis] Could not connect to Redis after 15 attempts. Exiting now.');
                return process.exit(1);
            }
        }
        self.setRedisConnectionTimeout();
...
XCSRedisClass.prototype.set = function set(req, key, value, cb) {
    ...
    if (self.redisClient && !self.redisClient.disableCaching) {
        ...
        if (unitTestUUID) {
            konsole.debug(req, '[Redis - setex] ' + key + ' -> ' + short_value + ' (' + k.XCSUnitTestTTLInSeconds + ' seconds)');
    ...
    if (self.redisClient && req && !self.redisClient.disableCaching) {
        if (doc_type) {
            ...
            if (!key) {
                ...
                xcsutil.logLevelDec(req);
                return self.redisClient.hget(key, url, cb);
            }
        } else {
            xcsutil.logLevelDec(req);
            return xcsutil.safeCallback(cb, {
