...
handlebars.registerHelper('override_toolchain', function (integration) {
    if (integration.bot.configuration && integration.bot.configuration.overrideToolchain) {
        return new handlebars.SafeString('<br /><span>Toolchain: ' + integration.bot.configuration.overrideToolchain.displayName + ' (' + integration.bot.configuration.overrideToolchain.identifier + ')</span>');
    }
    return "";
});
...
handlebars.registerHelper('issues_originators', function () {
    ...
    function createCreatorIssuesMessage(contributorIssues, type) {
        var contributorIssuesCount = 1;
        var contributorIssuesCount = Object.keys(contributorIssues).length;
...
handlebars.registerHelper('commit_date', function (timestamp) {
    if (timestamp !== undefined && timestamp !== null) {
        ...
        if (!isNaN(date)) {
            ...
            return months[date.getMonth()]+" "+ date.getDate() +", "+ date.getFullYear() +" "+ hours+":"+minutes+":"+seconds+" "+ampm+" GMT"+offset;
        }
        else {
