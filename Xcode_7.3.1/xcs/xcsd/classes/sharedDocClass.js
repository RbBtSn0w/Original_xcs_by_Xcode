...
XCSSharedDocClass.prototype.printSettingsFromCouchAndRedis = function printSettingsFromCouchAndRedis(req, cb) {
    ...
    dbCoreClass.findDocumentsWithQuery(req, k.XCSDesignDocumentSettings, k.XCSDesignDocumentViewAllSettings, {
        ...
        if (err && err.status !== 404) {
            log.error('[DEBUG - printSettingsFromCouchAndRedis - CouchDB] error:', err);
        } else {
            if (docs.length > 0) {
                couchDBDoc = docs[0];
            }
        ...
        redisClass.get(req, k.XCSDesignDocumentSettings, function (err, reply) {
            if (err) {
                log.error('[DEBUG - printSettingsFromCouchAndRedis - Redis] error:', err);
            } else {
                if (reply) {
            ...
            if (couchDBDoc && redisDoc) {
                if (couchDBDoc[k.XCSServiceEnabledKey] !== redisDoc[k.XCSServiceEnabledKey]) {
                    log.error('*****************************[ HIT BUG!: <rdar://problem/19376420> Can\'t add Xcode Server in Xcode preferences ]***************************************');
                    log.error('[DEBUG - printSettingsFromCouchAndRedis] Settings doc is NOT in sync!!!');
                    log.error('[DEBUG - printSettingsFromCouchAndRedis - CouchDB] settings:', JSON.stringify(couchDBDoc, null, 4));
                    log.error('[DEBUG - printSettingsFromCouchAndRedis - Redis] settings:', JSON.stringify(redisDoc, null, 4));
                    log.error('*****************************[ HIT BUG!: <rdar://problem/19376420> Can\'t add Xcode Server in Xcode preferences ]***************************************');
                }
            }
