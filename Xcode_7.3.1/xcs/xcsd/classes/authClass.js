...
XCSAuthClass.prototype.respondWithError = function respondWithError(req, res, err) {
...
XCSAuthClass.prototype.requireClientCertificate = function requireClientCertificate(req, res, next) {
    ...
    verifyClientCertificate_internal(req, function AUTHRequireClientCertificateCallback(err) {
        if (err) {
            ...
            return self.respondWithError(req, res, err);
        } else {
...
XCSAuthClass.prototype.force_login = function force_login(req, res) {
    ...
    function continueWithForceLogin() {
        ...
        xcsutil.clearRequestWatcherTimeout(res);
        self.respondWithError(req, res, new Errors.Unauthorized('Unauthorized'));
    }
...
function isUserInList(req, username, list, listName, strictCheck) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        if (strictCheck) {
            if (username === item) {
                log.debug('User', username, 'found in list', listName + ': access granted.');
                return true;
            }
        } else {
            if (username === item) {
...
XCSAuthClass.prototype.authorizeUserToCreateRepositories = function authorizeUserToCreateRepositories(req, username, cb) {
    ...
    authorize_internal(req, k.XCSCanCreateHostedRepositories, username, false, function AUTHAuthorizeUserToCreateRepositoriesCallback(err) {
        if (err) {
            err.message = 'Error authorizing user ' + username + ' to create hosted repositories: ' + JSON.stringify(err);
        }
        return xcsutil.safeCallback(cb, err);
    });
...
XCSAuthClass.prototype.enforceHostedRepositoryCreatorRole = function enforceHostedRepositoryCreatorRole(req, res, next) {
    var log = logger.withRequest(req),
        functionTitle = '[Auth - enforceHostedRepositoryCreatorRole] enforceHostedRepositoryCreatorRole';

    log.info('Requiring hosted repository creator role for this request.');
    ...
    if (user) {
        ...
        authProvider.authenticate(req, username, password, function AUTHParseBasicAuthHeaderAndAuthenticateAuthenticateUser(err) {
            if (err) {
                ...
                xcsutil.safeCallback(cb, new Errors.Unauthorized('Could not login because the username or password provided was incorrect.'));
            } else {
        ...
        var err = new Errors.Unauthorized('Could not login because no username was provided.');
