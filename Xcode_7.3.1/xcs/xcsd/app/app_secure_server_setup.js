...
module.exports = function app_secure_server_setup_init(app, cb) {
    if (cluster.isWorker || cluster.isDisabled) {
        ...
        if (!fs.existsSync(keyPath)) {
            message = 'Server SSL private key missing, skipping SSL setup.';
        ...
        if (!fs.existsSync(certPath)) {
            message = 'Server SSL certificate key missing, skipping SSL setup.';
            logger.error(message);
            return xcsutil.safeCallback(cb, {
                status: 500,
                message: 'Internal Server Error (xcsd): ' + message
        ...
        if (!fs.existsSync(clientCertsPath)) {
            message = 'Client SSL certificate authority missing, skipping SSL setup.';
            logger.error(message);
            return xcsutil.safeCallback(cb, {
                status: 500,
                message: 'Internal Server Error (xcsd): ' + message
