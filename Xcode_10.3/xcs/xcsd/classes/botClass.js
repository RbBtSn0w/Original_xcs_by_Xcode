...
XCSBotClass.prototype.diffConfiguration = function diffConfiguration(req, oldBot, newBot) {
    const log = logger.withRequest(req);
    ...
    diffKeys.forEach(key => {
        let before = oldConfig[key],
            after = newConfig[key];
        if (!isEqualForDiff(before, after, key)) {
            diffs[key] = {
                before, after
            };
        }
...
function isEqualForDiff(before, after, key) {
    return _.isEqual(sanitizeForDiff(before, key), sanitizeForDiff(after, key));
}

function sanitizeForDiff(value, key) {
    if (key === 'sourceControlBlueprint') {
        let newValue;
        try {
            newValue = bridge.sourceControl.transformBlueprintSync(value, {
