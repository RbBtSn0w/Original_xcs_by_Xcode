...
var blueprintOptionsConstants = {
    ...
    domainCredentials: $.DVTSourceControlWorkspaceBlueprintIncludeDomainAuthenticationCredentials,
    additionalValidationRepositories: $.DVTSourceControlWorkspaceBlueprintIncludeAdditionalValidationRemoteRepositories
};
...
exports.preflight = function (blueprint, callback) {
    ...
    var blueprintDictionary = objc.convertToNSObject(blueprint);
        ...
        try {
            ...
            if (!blueprint) {
                return done(callback, {
                    status: 400,
                    message: 'Could not initialize blueprint from dictionary representation.'
                });
            }
    
            var cbID;
            var cb = $(function (self, error) {
                unregisterCallback(cbID);

                try {
                    var errorCount = error ? error('objectsToErrors')('count') : 0;
                    logger.debug('Validated SCM authentication. Error?', error !== null, 'Error count:', errorCount);
        
                    var result = $.XCSBlueprintOperationResult('alloc')('init');
        
                    if (errorCount > 0) {
                        objc.keyedForEach(error('objectsToErrors'), function (repoError, repo) {
                            result('addError', repoError, 'forRepositoryID', blueprint('serializedIdentifierForRepository', repo));
                        });
                    } else if (error) {
                        logger.error('Error validating credentials: ', error('localizedDescription')('UTF8String'));
                        result('setGlobalError', error);
                    }
                    ...
                    if (!jsonResult) {
                        return done(callback, {
                            status: 500,
                            message: 'Could not serialize preflight response to JSON.'
                        });
                    }
        
                    done(callback, null, jsonResult);
                } catch (e) {
                    done(callback, objc.exception(e));
                }
...
exports.listBranches = function (blueprint, skipRepositoryIdentifiers, callback) {
    ...
    // get the JSON version of the blueprint
    var blueprintDictionary = objc.convertToNSObject(blueprint);
    var skipRepositoryIdentifiersArray = objc.convertToNSObject(skipRepositoryIdentifiers);
    ...
    withTemporaryDirectory(function (sshKeyURL, done) {
        try {
            ...
            var cbID;
            var cb = $(function (self, locations, error) {
                unregisterCallback(cbID);

                try {
                    var errorCount = error ? error('objectsToErrors')('count') : 0;
                    logger.debug('Listed remote branches. Error?', error !== null, 'Error count:', errorCount);
                    ...
                    if (errorCount > 0) {
                        ...
                        logger.error('Error listing remote branches: ', error('localizedDescription')('UTF8String'));
                        result('setGlobalError', error);
                    }
            ...
            blueprint('validateAuthenticationAndListBranchesSkippingRepositoryIdentifiers', skipRepositoryIdentifiersArray, 'completionBlock', cb);
        } catch (e) {
            done(callback, objc.exception(e));
        }
    });
...
exports.merge = function (existing, merge, callback) {
    ...
    var existingBlueprintDict = objc.convertToNSObject(existing),
        mergeBlueprintDict = objc.convertToNSObject(merge);
    ...
    withTemporaryDirectory(function (sshKeyURL, done) {
        try {
            ...
            var flags = blueprintOptionsToFlags({
                revisions: true,
            ...
            if (!jsonDict) {
                return done(callback, {
                    status: 500,
                    message: 'Could not serialize blueprint to JSON.'
                });
            }
...
exports.getMissingCredentials = function (blueprint, credentials, callback) {
    ...
    var existingBlueprintDict = objc.convertToNSObject(blueprint),
        mergeBlueprintDict = objc.convertToNSObject(credentials);
...
exports.transformBlueprintSync = function (blueprint, options) {
    var transformed = null,
        error = null;

    exports.transformBlueprint(blueprint, options, (err, bp) => {
        error = err;
        transformed = bp;
    });
