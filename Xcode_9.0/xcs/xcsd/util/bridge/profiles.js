...
objc.importSharedFramework('DVTFoundation');
objc.importSharedFramework('DVTDeviceFoundation');
objc.importSharedFramework('DTDeviceKitBase');
...
exports.getProfileInfo = function getProfileInfo(filePath) {
    return Promise.try(() => {
        ...
        let profile = $.DTDKProvisioningProfile('profileWithData', data, 'error', errorRef);
        if (!profile) {
        ...
        return {
            name: (profile('name') || empty)('UTF8String'),
            teamID: (profile('teamID') || empty)('UTF8String'),
            teamName: (profile('teamName') || empty)('UTF8String'),
            identifier: (profile('UUID') || empty)('UTF8String')
        };
    });
};
