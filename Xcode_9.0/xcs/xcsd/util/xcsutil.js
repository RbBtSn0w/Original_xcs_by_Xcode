...
xcsutil.hardwareInfo = function hardwareInfo(req, res) {
    ...
    xcsutil.standardizedResponse(res, 200, {
        computerName: hardware.getComputerName(),
        hardwareUUID: hardware.getHardwareUUID()
    });
};
...
const waitingForRestartTask = {
    title: "Waiting for service to restart.",
    completed: 0
};
...
xcsutil.maintenanceTasks = function status(req, res) {
    ...
    xcsutil.maintenanceTasks_internal(function (err, maintenanceTaskList) {
        if (err) {
            ...
            if (maintenanceTaskList.length > 0) {
                log.info('Found', maintenanceTaskList.length, 'maintenance tasks running.');
                return xcsutil.standardizedResponse(res, 532, maintenanceTaskList);
            } else {
                if (require('../classes/worker.js').isInitPhase) {
                    log.info("No maintenance tasks are running, but the workers haven't restarted yet.");
                    return xcsutil.standardizedResponse(res, 532, [waitingForRestartTask]);
                } else {
