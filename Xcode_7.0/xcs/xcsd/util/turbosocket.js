    ...
    if (type === 'helo') {
        ...
        if (this.readingRandomDataLength) {
            ...
            fs.open('/dev/random', 'r', function (err, fd) {
                if (err) {
                    konsole.log(null, '[XCSNode TurboSocket] Error opening random device: ' + JSON.stringify(err));
                } else {
                    var buf = new Buffer(len);
                    fs.read(fd, buf, 0, len - 5, null, function (err, bytesRead, b) {
                        if (err) {
                            konsole.log(null, '[XCSNode TurboSocket] Error occurred reading random bytes: ' + JSON.stringify(err));
